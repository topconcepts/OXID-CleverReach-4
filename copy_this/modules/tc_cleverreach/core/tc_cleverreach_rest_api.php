...
class tc_cleverreach_rest_api extends oxUBase
{
    private $attributes;
    ...
    public function __construct($shopId = null)
    {
        ...
        $this->listId           = oxRegistry::getConfig()->getShopConfVar('tc_cleverreach_list_id', $shopId);
        ...
        if ($this->tc_isOAuthTokenValid($shopId)) {
            ...
            echo($shopmsg.'oAuth Zugangs-Token ist ungültig. Bitte authentifizieren Sie sich erneut.');
        }
    }
            ...
            if ($header["http_code"] < 200 || $header["http_code"] >= 300) {
                ...
                if ($this->throwExceptions) {
                    if (strstr($message, 'invalid group')) {
                        throw new tc_group_not_found_exception('Group not found.');
                    } else {
                        throw new tc_cleverreach_exception('' . $header["http_code"] . ';' . $message);
                    }
                }
                $in = null;
            }
     ...
     *
     * @param tc_cleverreach_collection $collection
     *
     * @return array
     * @throws Exception
     */
    public function receiverAddBatch($collection)
    {
        ...
        $result = (array)$this->post("/groups/{$this->listId}/receivers/upsert", $request);

        foreach ($result as $row) {
            if (is_string($row)) {
                if (strstr($row, 'invalid address')) {
                    throw new oxException('Export Failed.');
                }
            }
        }
     ...
     *
     * @param tc_cleverreach_collection_filter $filter
     *
     * @return array
     * @throws Exception
     */
    public function extractFilter(tc_cleverreach_collection_filter $filter)
    {
        ...
        foreach ($filter as $element) {
            ...
            $hasOrderFlag = oxRegistry::getConfig()->getShopConfVar(tc_cleverreach_config::SETTING_ORDER_FLAG);
     ...
     * @return mixed
     */
    public function isListValid($id)
    {
        $result = $this->getListById($id);
    ...
    public function getListById($id)
    {
        ...
        } catch (tc_cleverreach_exception $e) {
