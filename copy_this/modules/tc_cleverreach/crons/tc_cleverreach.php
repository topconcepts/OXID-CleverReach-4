...
function setOrderOption($shopId=null)
{
    if ((is_array($_SERVER['argv']))) {
        oxRegistry::getConfig()->saveShopConfVar(
            'bool',
            tc_cleverreach_config::SETTING_ORDER_FLAG,
...
try {
    ...
    // tc_jobinstance
    try {
        /* @var $instance tc_jobinstance */
        $instance = oxNew('tc_jobinstance', 'tc_cleverreach');
        $instance->initialize();
    } catch (tc_jobinstance_exception $e) {
        throw $e;
    } catch (Exception $e) {
        throw new oxException('tc jobinstance ist nicht richtig installiert:' . $e->getMessage(), 40);
    }
    ...
    try {
        $notConnectedShops = array();
        /* @var $transfer tc_cleverreach_transfer */
        $transfer = oxNew('tc_cleverreach_transfer',null, $transferType, true);
        ...
        throw new oxException('tc CleverReach ist nicht richtig installiert:' . $e->getMessage(), 41);
    }
    ...
    if (!$bCSV && $transfer->transferPossible() === false)
        throw new tc_cleverreach_exception('API Key oder Listen ID fehlt', 42);

    $transferResult = array();
    $i      = 0;

    $shops = oxDB::getDb()->getCol('SELECT oxid FROM oxshops WHERE oxactive = 1');

    foreach ($shops as $shopId) {
        $counter = 0;
        $hasInfo = false;
        $list = oxRegistry::getConfig()->getShopConfVar('tc_cleverreach_list_id', $shopId);
        if($bCSV || (!empty($list) && tc_cleverreach_oauth::tc_isOAuthTokenValid($shopId))) {
