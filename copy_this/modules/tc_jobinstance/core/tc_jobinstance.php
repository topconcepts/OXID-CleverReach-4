...
class tc_jobinstance {
    ...
    public function initialize() {
        ...
        if (is_file($this->sFile) === true) {
            ...
            $oPid  = oxNew('tc_jobinstance_pid', $aInfo['pid']);
            ...
            if ($oPid->isRunning() === true) {
                ...
                if ($aInfo['maxExecutionTime'] !== null && (time() - $aInfo['time']) > $aInfo['maxExecutionTime']) {
                    $oPid->kill();
                    throw new tc_jobinstance_exception('max execution of programm reached. running since ' . date('Y-m-d H:i:s', $aInfo['time']), tc_jobinstance_exception::ERROR_MAX_EXECUTION_REACHED);
                }

                $this->blAlreadyRunning = true;
                throw new tc_jobinstance_exception('Job-Instance already running!', tc_jobinstance_exception::ERROR_ALREADY_RUNNING);
            }

            // wenn nicht läuft, datei löschen
            if (unlink($this->sFile) === false) {
                throw new tc_jobinstance_exception('process not running and pid-file not deleteable!', tc_jobinstance_exception::ERROR_CAN_NOT_START);
            }
        }
